# DES - discrete event (mini) simulator
...
class QueuedEvent(Event):
      ...
      def out(self,sim):              # pass to connected objects
            if self.customer!=None:
                  if "value" not in self.customer.attr:
      ...
      def _eval(self,code,n=-1):
            ...
            for code in codes:
                  ...
                  if len(cc)>1:
                        ...
                        cc[1] = "=".join(cc[1:])  # rejoin
                        if self.customer!=None:
                              for k,v in self.customer.attr.items():
                                    cc[1] = cc[1].replace(k,str(v))
...
class Generator(BpmnEvent):
      ...
      def exec(self, sim):
            ...
            self.time += (self.fun!=None and self.fun(self.param) or self.param)
            if self.tmax>0 and self.time <= self.tmax or self.n<-self.tmax-1:
                  self.n += 1
                  sim.add(self)       # next one in random time
class Service(BpmnEvent):
      def __init__(self, fun, param, code=None, N=1):
            BpmnEvent.__init__(self,"Activity",code) 
            self.fun, self.param, self.N = fun, param, N
      def insert(self, cust, sim):
            if self.customer == None : # if free add to simulator with end time
                  self.customer = cust
                  self.time = sim.now() + (self.fun!=None and self.fun(self.param) or self.param)
                  _print("#"+str(self.param)+" starts serving "+
                  ...
                  if isinstance(self,Timer):  # modify required final execution time
      ...
      def exec(self, sim):
            if self.customer!=None:
                  self.customer.attr["__t"+str(self.id)+"e"]=sim.now()
                  self.A["A.n"] += 1
            _print("#"+str(self.param)+" finished serving " + str(self.customer) + " at " + str(self.time))
            if self.code!=None :      
                  self._eval(self.code)   # evaluate script code ([var]=<value>)
            if self.customer!=None:
                  self.customer.attr["__n"+str(self.id)] += 1
                  if self.customer.attr["__n"+str(self.id)] == self.N:
                        self.out(sim)     # pass customer to connected object
            self.customer = None    # mark that now the service is free !!!
            if len(self.queue)>0 :  # but if anybody in queue
                  self.insert(self.queue.pop(), sim)  # get and insert into simulator
...
# ---- derived classes (Service derived) ----
class ScriptGate(Service):
      def __init__(self,code=None,n=1):
            Service.__init__(self,None,0,code,n)
# ---- bpmn double derived classes (ScriptGate or Service derived) ----
class XorGate(ScriptGate):    # random output if two outputs 
      def __init__(self,code="=B(0.5)"):
            ScriptGate.__init__(self,code)
            self.setName("exclusiveGateway")
class AndGate(ScriptGate):    
      def __init__(self,code=None,n=1): # n - number of inputs to wait
            ScriptGate.__init__(self,code,n)
            self.setName("parallelGateway")
      def insert(self, cust, sim):
            self.N=len(self.prev())     # set number of inputs to wait
            ScriptGate.insert(self,cust,sim)
class Start(Generator):
      def __init__(self,fun,param,tmax=10.0,code=None):
...
class Task(Service):
      def __init__(self,fun,param,code=None):
            ...
            self.setName("task")
class Timer(Service):
      def __init__(self,fun,param=None,code=None):
...
def attr_tostring(a):
      ...
      return s
def attr_tosvgstring(a,w=16,h=24):
      s = ""
      c=['red','green','cornflowerblue','coral','mediumseagreen','mediumpurple']
      for i in range(100):
            t="__t"+str(i)
            if t+"b" in a:
                  s+='<text x="8" y="'+str(32+h*i)+'">'+str(i)+'</text>\n'
                  t0,t1,t2 = a[t+"a"],a[t+"b"],a[t+"e"]
                  s+='<rect y="'+str(20+h*i)+'" x="'+str(20+w*t1)
                  s+='" width="'+str(t2-t1==0 and 1 or w*(t2-t1))+'" height="'+str(h-2)+'" stroke="black" fill="'
...
def to_svg(ee=QueuedEvent.instances):
      ...
      s+= '<rect x="20" y="10" width="1600" height="2"/>\n'
      for i in range(100):
      ...
      for i in range(len(ee)-1,-1,-1): # move previous to the level of its first next
            ...
            if len(ep)>0 and ep[0].output[0]==ee[i] and ep[0].pp[0]==ee[i].pp[0]-1:
                  if ep[0].pp[1]!=ee[i].pp[1]:
                        ep[0].pp[1]=ee[i].pp[1]
      """
      for i in range(len(ee)):   # set y position to the level of previous 
            e = ee[i].prev()
...
def to_bpmn(ee=QueuedEvent.instances,pp=None):
      ...
      for i in range(len(ee)):
            ...
            if "task" in type:
                  ...
                  if len(ee[i].prev())>1: type = "receiveTask"
            ...
            if "->" in s1: # connection definition
                  for s1a in s1.split(";"):
                        ...
                        if len(code)>1:
                              ...
                              connect(ee[int(code[0])-1],ee[int(code[1])-1])
            else:          # event definition
                  code=s1.split(" ")
                  if len(code)>1:
                        ee.append(eval(" ".join(code[1:])))
      return ee
def to_string(ee):
      s=''
      for e in ee:
            ...
            for j in range(len(it)):
                  ...
                  if w==0: # bpmn flow
                        ...
                        if "Event" in it.attrib["id"]:
                              ...
                              s+= '<ellipse cx="'+str(x+w/2)+'" cy="'+str(y+h/2)+'" rx="'+str(w/2)+'" ry="'+str(h/2)+'" stroke-width="'+stroke+'" stroke="'+color+'" fill="white"><title>'+id2+'</title></ellipse>\n'
                              if "intermediate" in it.attrib["id"]:
                                    s+= '<ellipse cx="'+str(x+w/2)+'" cy="'+str(y+h/2)+'" rx="'+str(w/2-3)+'" ry="'+str(h/2-3)+'" stroke-width="'+stroke+'" stroke="darkgreen" fill="none"/>\n'
                                    if "Catch" in it.attrib["id"]:
                                          if "ondition" in it.attrib["id"]:
                                                ...
                                                s+= '<ellipse cx="'+str(x+w/2)+'" cy="'+str(y+h/2)+'" rx="'+str(w/2-6)+'" ry="'+str(h/2-6)+'" stroke-width="'+stroke+'" stroke="black" fill="none"/>\n'
                              ...
                              if "erminate" in it.attrib["id"]:
                                    s+= '<ellipse cx="'+str(x+w/2)+'" cy="'+str(y+h/2)+'" rx="'+str(w/2-6)+'" ry="'+str(h/2-6)+'" stroke-width="'+stroke+'" stroke="red" fill="red"/>\n'
...
2 Task(U,[2.0,3.0])    # a task with execution time from 2 to 3 units (uniform distributed)
3 Task(U,[1.0,3.0])
4 End()
1->2; 2->3; 3->4
"""
...
19 End()
1->2; 2->4; 4->6; 6->7; 6->8; 6->9; 6->10; 7->11; 8->11; 9->11; 10->11;
11->12; 12->13; 12->14; 12->15; 13->16; 14->16; 15->16; 16->17; 17->18;
18->19; 1->3; 3->5;
"""
ex8a="""
1 Start(E,[2.0],20.0)
...
11 AndGate()
12 End()
1->2; 2->3; 3->4; 3->5; 4->6; 5->6; 6->7; 7->8; 7->9; 7->10; 8->11; 9->11; 10->11; 11->12
"""
ex9="""
...
13 XorGate()
14 End()
1->2; 2->3; 2->4; 3->5; 4->5; 5->6; 6->7; 7->8; 8->9; 8->10; 9->11;
10->13; 11->12; 12->13; 12->6; 13->14
"""
def main_fun(exn,n): # string representation, number of simulation
      ee,data=[],[]  # list of events, array of results
      for _ in range(n):            
            ne = EventNetwork(exn)
            s = Simulator()
            for e in ne:
            ...
            if _==0:
                  print(to_bpmn(ne.ee),file=open('des.bpmn','w'))
      ...
      for e in ne:
            if isinstance(e,End):
                  print(e.id,"(%d):"%len(e.queue),e.queue)  # snk
...
6 XorGate() 
7 End()
1->2; 2->3; 2->4; 3->6; 4->5; 5->6; 6->7
"""
if __name__=="__main__":
      import sys
      exn = len(sys.argv)>1 and from_file(sys.argv[1]) or eval('ex'+str(5))
      n = len(sys.argv)>2 and int(sys.argv[2]) or 1
      #print(exn)
      
      ee = main_fun(exn,n)
      #for nn in [1,2,3,4,5,6,7,8,9]: main_fun(eval('ex'+str(nn)),n)
