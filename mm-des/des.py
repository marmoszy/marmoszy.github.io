...
if __name__=="__main__":
      import sys
      s = len(sys.argv)>1 and from_file(sys.argv[1]) or eval('ex'+str(28))
...
Servers are the processes of getting infected and recovering.
The durations people stay in the servers represent the process of getting
infected and getting recovered. Customers are susceptible people to be
infected, and infected people waiting to recover.
There are two queues in this model. As susceptible agents are created,
queues for infection are formed while people are waiting to be infected.
On the other hand, as people get infected, they form a second queue waiting
to recover. During each iteration (dt), each agent in queue has
a probability equals (Infraction Rate * number of infected * dt) to get
infected. Each infected agent has a probability to recover = RecoveryRate.
After agents recover, they enter the sink of recovered people.
--------------     _____----------     _____-----------    -------------
|Susceptibles| -> |queue| Infect | -> |queue| Recover | -> | recovered |
--------------          ----------          -----------    -------------
"""
